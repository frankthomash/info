 ...
 */
export const databaseParams = Object.freeze({
  VSS: {
...
const useDefaultString = (defaultValue: string): string => {
  console.info(`Using default: ${defaultValue}`);
  return defaultValue;
};
const useDefaultNumber = (defaultValue: number): number => {
  console.info(`Using default: ${defaultValue}`);
  return defaultValue;
};
 ...
 */
const getDatabaseConfig = (): IotDBConfig => {
  const iotdb_config: IotDBConfig = {
    ...
    timeZoneId: getEnvValue("IOTDB_TIME_ZONE_ID") || Intl.DateTimeFormat().resolvedOptions().timeZone,
  };
 ...
 */
export const databaseConfig: Partial<IotDBConfig> =
