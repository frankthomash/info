...
const dataTypeProcessors: {
  [key in IoTDBDataType]: (bytes: Uint8Array | Buffer) => number | bigint | string;
} = {
  ...
  private constructRowRecordFromValueArray(): { [key: string]: any } {
    ...
    for (let i = 0; i < this.iotdbRpcDataSet.getColumnSize(); ++i) {
      ...
      if (!this.iotdbRpcDataSet.isNullByIndex(dataSetColumnIndex)) {
        ...
        const dataType = this.iotdbRpcDataSet.getColumnTypeDeduplicatedList()[location];
        const tsName = this.iotdbRpcDataSet.findColumnNameByIndex(dataSetColumnIndex);
